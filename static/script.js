...
function renderTable() {
  ...
  if (filteredProducts.length === 0) {
    table.innerHTML = '<tr><td colspan="9" class="text-center text-muted">Товарів не знайдено</td></tr>';
    return;
  }
  ...
  if (!searchTerm) {
    ...
    filteredProducts = productsData.filter(p => {
      return (
        ...
        p.warehouse_number.toLowerCase().includes(searchTerm) ||
        p.shelf.toLowerCase().includes(searchTerm) ||
        p.rack.toLowerCase().includes(searchTerm)
      );
    });
  }
...
window.addEventListener('load', function() {
  ...
  // Обробник пошуку (вводимо текст - пошук в реальному часі)
